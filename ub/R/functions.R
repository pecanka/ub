...
#' @name str_find_in
#' @export 
str_find_in_function = function(string, fun, envir = parent.frame(), trace = 0) {

  if(trace > 0 && is.character(fun)) {
    message("Searching for the string '",string,"' within '", fun, "()' ...")
  }
  ...
  src_code = tryCatch(print2var(fun), error = function(x) FALSE)
  
  grep(string, src_code)
  
}

#' @rdname str_find_in
#' @export 
str_find_in_package = function(string, pckg, what = 'all', trace = 0) {

  if(trace > 0) {
    message("Listing function in the package '",pckg,"' ...") 
  }
  
  funs = list_package_objects(pckg, mode = 'function', what = what, quiet = TRUE)$object_name
  names(funs) = funs
  
  locs = lapply(funs, str_find_in_function, string = string, envir = asNamespace(pckg), trace = trace)
...
#' f(1:5, 3:10)
#'
#' @family function-modification functions provided by ub
#' @export
