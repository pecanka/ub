...
#'
#' `all_subsets_native()` is an alternative to the `all_subsets()` that avoids
#' `base::expand.grid()` and is slightly faster.
#'
#' @export
...
#' @export
all_subsets_native <- function(set) {

  n = length(set)
  bin = vector(mode = "list", length = n)

  for (i in 1L:n) {
    bin[[i]] <- rep.int(c(rep.int(FALSE, 2L ^ (i - 1L)), rep.int(TRUE, 2L ^ (i - 1L))), 2L ^ (n - i))
  }
