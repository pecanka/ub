...
function_kidnap = function(fun_name, envir=parent.frame(), msg_argument, restore_call, remodify=TRUE, 
  ...
  calls = c(
    ...
    paste0("  message('  - input \\'i\\' and press <ENTER> to ignore the call to `",fun_name,"()`",
           "          and continue as if it was never made');"),
    paste0("  message('  - input \\'b\\' and press <ENTER> to enter the browser inside the call to `",
                      fun_name,"()`');"),
