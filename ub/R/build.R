...
#'
#' `build_ub()` calls `build_package()` using the settings for 'ub' for a complete
#' creation (i.e., create, update documentation, build, install, attach)
#'
#' `install_ub()` calls `build_package()` using the settings for 'ub'.
#'
#' @export
build_package = function(pckg_name, pckg_dir = pckg_name, pckg_dir_source = 'source', path = '.', 
  ...
  if(add_path_to_global) {
    ...
    if(isNamespace('ub')) {
      ...
      ub::R_del_code_startup(substr(startup_code, 1, 30), file_Rprofile)
      ub::R_add_code_startup(startup_code, file_Rprofile)
    } else {
      msg("Package's installation path cannot be added to .GlobalEnv and to .Rprofile without the package 'ub'.")
    }
...
#' @export
build_ub = function(pckg_name = 'ub', pckg_dir = 'ub', pckg_dir_source = 'source', path = '.', 
  ...
  message("Note: Loading 'ub' might fail. Restart R prior to loading to avoid this.")

}

#' @rdname build_package
#' @export
install_ub = function(pckg_name = 'ub', pckg_dir = 'ub', path = '.', install_mode = 'full', 
