...
#'
#' @examples
#' package_is_installed('ub')
#' package_is_installed(ub)
#' lib = 'ub'; package_is_installed(lib)
#' lib = 'ub'; package_is_installed(lib, character.only=TRUE)
#'
#' @name llibrary
#'
#' @family package-related functions provided by ub
#' @export
...
unload_all_libraries = function() {
  ...
  assign('.ub_unloaded_libraries', unloaded, envir=.GlobalEnv)
...
reload_unloaded_libraries = function(pckgs) {
  ...
  if(missing(pckgs)) {
    pckgs = if(exists('.ub_unloaded_libraries', envir=.GlobalEnv)) {
      rev(get('.ub_unloaded_libraries', envir=.GlobalEnv))
    } else {
      NULL
    }
  }
...
#'
#' @examples
#' apply_pckg('rev_cols', 'ub', class)    # returns 'function'
#'
#' @family package-related functions provided by ub
#' @export
...
.eai = function(call_type=c('library','require')) {
  ...
  for(f in call_type) {
    ...
    assign(f, ub::llib, envir=.GlobalEnv)
  }
