#' Get today's date
#'
#' Returns today's date (in the current time zone) in the specified format.
#'
#' @param format Format of the output.
#' @param set_return_class A function that determines the class of the 
#'        returned date. Uses \code{base::as.character} by default but for full
#'        dates, while \code{base::as.Date} can be also useful.
#'
#' @examples
#' t_day()     # current day
...
#'
#' @family calendar functions provided by utilbox
#' @export
...
#'
#' Calculated the date of the previous day or any other day relative to today 
#' in the current time zone in the specified format. How many days are shifted
#' to the past is controlled via \code{lag}, which accepts vector input.
#' Negative values in \code{lag} shift the date to the past, positive 
#' values shift the date into the future.  
#'
#' @examples
#' y_day()          # yesterday
