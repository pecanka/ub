...
script_wrap_text = function(file, code, max_length=70, eol="\n#' ", punctuation='.!?', 
  ...
  wrap_line = function(Cj, orig_Cj) {
  
    Nj = nchar(Cj)
    
    # identify the positions of spaces for possible insertions of 'eol'
    w_space = unlist(unname(gregexpr('\\s+',Cj)))
    ...
    while(Nj>max_length && any(w_space>0) && attempt<=max_attempts) {
      ...
      Cj = str_replace(Cj, eol, w_last_all, str2_shift=nchar(eol)-1)
    ...
    if(attempt>max_attempts) {
      ...
      catn("Keeping the original line (unaltered).")
      Cj = orig_Cj
    }
  ...
  # wrap the lines
  Code_long = lapply(Code_long1, function(Cjs) unlist(lapply(Cjs,  wrap_line)))
  
  # shift the positions to reflect the cumulative insertions
  long = long + c(0, cumsum(h1(sapply(Code_long, length),-1) - 1))

  #Code0 = Code
  for(i in seq_along(long))
    Code = c(h1(Code, long[i]-1), Code_long[[i]], t1(Code, -long[i]))
    #Code = insert(Code, Code_long[[i]], long[i]-1, make_room=FALSE, drop_limit=0)
