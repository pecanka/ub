...
#' @family sequence-related functions provided by utilbox
#' @export
insert = function(x, what, after, count=1, make_room=TRUE, drop_limit=Inf) {
  
  if(missing(after)) after = length(x)
  
  stopifnot(length(after)==1, after>=0)
  
  start = head(x, after)
  middle = rep(what, count)

  ndrop = after + pmin(ifelse(make_room, 0, length(middle)), drop_limit)
  end = if(ndrop==0) x else tail(x, -ndrop)
